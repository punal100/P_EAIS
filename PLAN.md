...
- `Plugins/P_EAIS/DevTools/scripts/` — automation scripts: `RunEAISTests.ps1`, `ValidateAIJson.ps1`, `ValidateGraph.ps1`, `VerifyPlanConsistency.ps1`, `build_headless.bat`, `build_headless.sh`, `run_tests.sh`, `test_eais.bat`
...
## 8.3 JSON validation: `DevTools/scripts/ValidateAIJson.ps1`

```powershell
# From Plugins/P_EAIS (plugin root)
.\DevTools\scripts\ValidateAIJson.ps1
```

## 8.4 Test runner wrapper: `scripts/run_tests.sh`

- Runs PowerShell JSON validation (`ValidateAIJson.ps1`), then `build_headless` script, then `UnrealEditor-Cmd` automation tests. Fail fast on errors.
...
jobs:
  build-and-test:
    ...
    steps:
      ...
      - name: Validate JSONs
        run: powershell -NoProfile -ExecutionPolicy Bypass -File .\DevTools\scripts\ValidateAIJson.ps1
      - name: Build & Test
...
- `DevTools/scripts/` — automation scripts (build & tests).
...
- Existing AI assets (if any) should be migrated manually or via a converter tool added to `DevTools/scripts/convert_old_ai_to_eais.py`. Because backwards compatibility is not required, you may archive old assets into `~Archive/old_ai/`.
...
- **JSON validation failures:** run `.\DevTools\scripts\ValidateAIJson.ps1` and fix reported errors.
...
1. `.\DevTools\scripts\ValidateAIJson.ps1` exits 0 for provided example JSON behaviors.
...
3. Run `.\DevTools\scripts\ValidateAIJson.ps1` to confirm JSONs validate.
